import { sendMatchInfo, sendLobby } from '../lib/discord';
...
const initLobby = async () => {
    ...
    const somPreReadyFunc = (mutations) =>
      $.each(mutations, (i, mutation) => {
        ...
        if (preReadyButton && preReadyButton.length) {
          const som =
            opcoes.somPreReady === 'custom'
              ? opcoes.customSomPreReady
              : opcoes.somPreReady;
          const audio = new Audio(som);
          const volume = opcoes.volume || 100;
          audio.volume = volume / 100;
          document
            .getElementById('setPlayerReady')
    ...
    const somReadyFunc = (mutations) =>
      $.each(mutations, (i, mutation) => {
        ...
        if (
          ...
          const som =
            opcoes.somReady === 'custom'
              ? opcoes.customSomReady
              : opcoes.somReady;
          const audio = new Audio(som);
          const volume = opcoes.volume || 100;
          audio.volume = volume / 100;
          document
            .getElementById('gameModalReadyBtn')
            .addEventListener('click', function (e) {
              audio.play();
            });
        }
  ...
  if (opcoes.autoAceitarPreReady) {
    ...
    const autoAceitarPreReady = criarObserver(
      '#rankedModals',
      autoAceitarPreReadyFunc
    );
  }
  ...
  if (opcoes.autoAceitarReady) {
    ...
    const autoAceitarReady = criarObserver(
      '#rankedModals',
      autoAceitarReadyFunc
    );
  }

  if (opcoes.autoFixarMenuLobby) {
    let freeuser = document.getElementsByClassName(
      'SettingsMenu SettingsMenu--free'
    );
    ...
    const autoFixarMenuLobby = criarObserver(
      '#lobbyContent',
      autoFixarMenuLobbyFunc
    );
  }
  ...
  if (opcoes.autoConcordarTermosRanked) {
    ...
    const autoConcordarTermosRanked = criarObserver(
      '#rankedModals',
      autoConcordarTermosRankedFunc
    );
  }
    ...
    const partidaInfoFunc = (mutations) =>
      $.each(mutations, async (i, mutation) => {
        ...
        if (ipInput && ipInput.length) {
          ...
          console.log(campoIP);
          if (campoIP[0].value) {
            const listenGame = await axios.get(
              'https://gamersclub.com.br/lobbyBeta/openGame'
            );
            if (listenGame.data.game.live) {
              if (document.getElementById('botaoDiscordnoDOM')) {
                ...
                document
                  .getElementById('botaoDiscordnoDOM')
      ...
      mutations.forEach(async (mutation) => {
        ...
        for (let i = 0; i < mutation.addedNodes.length; i++) {
          ...
          if (
            ...
            node.nextElementSibling.className.includes(
              'sidebar-desafios sidebar-content'
            )
          ) {
              ...
              if (document.getElementById('discordLobbyButton')) {
                ...
                document
                  .getElementsByClassName(
                    'sidebar-titulo sidebar-sala-titulo'
                  )[0]
                ...
                document
                  .getElementById('discordLobbyButton')
...
function adicionarBotaoCancelarCriarLobby() {
  $(
    '#lobbyContent > div.row.lobby-rooms-content > div > div > div:nth-child(3)'
  ).html(
  ...
  document
    .getElementById('cancelarCriacaoLobbyBtn')
...
function adicionarBotaoForcarCriarLobby() {
  ...
  document
    .getElementById('forcarCriacaoLobbyBtn')
    .addEventListener('click', function () {
...
function intervalerCriacaoLobby() {
  return setInterval(async () => {
    if (!$('.sidebar-titulo.sidebar-sala-titulo').text().length) {
      const lobbies = $(
        '.LobbiesInfo__expanded > .Tag > .Tag__tagLabel'
      )[0].innerText.split('/')[1];
      ...
      if (Number(lobbies) < Number(limiteLobby)) {
        ...
        chrome.storage.sync.get(['preVetos', 'lobbyPrivada'], async (res) => {
          ...
          const criarPost = await axios.post(
            '/lobbyBeta/createLobby',
            postData
          );
          if (criarPost.data.success) {
            ...
            if (
              criarPost.data.message.includes('Anti-cheat') ||
              criarPost.data.message.includes('banned')
            ) {
              clearInterval(intervalCriarLobby);
              adicionarBotaoForcarCriarLobby();
