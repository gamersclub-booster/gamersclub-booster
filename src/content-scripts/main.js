...
const initGcBooster = async () => {
  if (generalOptions.mostrarLevelProgress) {
    ...
    const playerInfo = await grabPlayerLastMatch(
      `https://${GC_URL}/players/get_playerLobbyResults/latest/1`
    );
    ...
    const progressBar = maxPontos
      ? `${((currentRating - minPontos) / (maxPontos - minPontos)) * 100}%`
      : '100%';

    const strText =
      playerNextLevel > 20 ? '' : 'Skill Level ' + playerNextLevel;
    const nextLvl = playerNextLevel > 20 ? '' : playerNextLevel;

    const colorTxt = rating_points.includes('-')
      ? 'color: #ef2f2f;'
      : 'color: #839800;';
    ...
    const subscriberStyle =
      isSubscriber === 'true' ? 'subscriber' : 'nonSubscriber';
    $('.MainHeader__navbarBlock:last')
      ...
      rating_points.includes('-') ? rating_points : '+' + rating_points
    }</span></a></div>
      ...
      levelColor[playerLevel]
            ...
            <span title="${strText}" style="cursor: help;display: ${
      playerNextLevel > 20 ? 'none' : 'inline-block'
    }">
            ...
            <span title="${strText}" style="cursor: help;display: ${
      playerNextLevel > 20 ? 'inline-block' : 'none'
    }">
